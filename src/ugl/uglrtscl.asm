...
uglPutScl       proc    public uses bx si di ,\
	...
	ifdef	_DEBUG_
		CHECKDC	fs, @@exit, uglPutScl: Invalid dst DC
                ...
                ;; vtxb[i].x = (vtxa[i].x + xh) * xScale + x
                ;; vtxb[i].y = (vtxa[i].y + yh) * yScale + y
                xor     si, si
                mov     cx, 4

@@loop:         fld     vtxa[si].x              ;; x
                fadd    xh                      ;; x
                fmul    xScale                  ;; x*xScale
                fiadd   x                       ;; x*xScale+x
                fstp    vtxb[si].x              ;;
                
                fld     vtxa[si].y              ;; y
                fadd    yh                      ;; x
                fmul    yScale                  ;; y*yScale
                fiadd   y                       ;; y*yScale+y
                fstp    vtxb[si].y              ;;
...
;;::::::::::::::                
uglPutMskScl    proc    public uses bx di si,\
                        dstDC:dword,\
                        x:word,\
                        y:word,\
                        xScale:real4,\
