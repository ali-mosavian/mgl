...
mem_RdBegin     proc    near public uses ax
...
mem_WrBegin     proc    near public uses ax
...
;;	ax= /	     /    /	     (read  /     )
mem_RdWrBegin	proc    near public uses ax
                mov     ax, W fs:[DC_addrTB][di]
                mov     bx, O mm$memCtx.wrCurrent
		mov	ss:mm$memCtx.wrCurrent, ax
		mov	es, ax
...
;;  in: bx-> gfxCtx[dst.type]
;;      edi= dst.addrTb[y].segm
mem_RdWrSwitch  proc    near public
		mov	ss:mm$memCtx.wrCurrent, di
		mov	es, di
		mov	ax, di
		ret
...
;;	ax= /   /	seg (read  /     )
mem_RdWrAccess	proc    near public
                mov	edi, fs:[DC_addrTB][di]
		mov     es, di
		mov	ax, di
                shr     edi, 16
...
;;
;; out: ds:si-> src framebuffer
mem_FullAccess  proc    near public
