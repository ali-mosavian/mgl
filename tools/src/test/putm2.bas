'' uglPutMsk speed test (dcs allocated in same seg/ems page by uglNewMult)
...
'':::
        ...
        if parseCmmLine(bpp, dct, mhz) < 3 then
                ExitError "usage: putm2 bpp dct mhz"
        end if
	...
	case "ems"
        ...
        '' fill the bitmaps
        For i = 0 To BMPS-1
                uglClear memBmp(i), uglColor(cFmt,255,0,255)
                'uglClear emsBmp(i), uglColor(cFmt,255,0,255)
                For y = 0 To bmpH-1
                        x1 = Rnd * bmpW
                        x2 = Rnd * bmpW
                        clr& = Rnd * colors&
