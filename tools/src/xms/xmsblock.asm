;;
;; xmsBlock.asm -- helper functions to find and split heap's blocks
;;
...
xm$BlockFind    proc    near public uses bx ecx edx es
                ...
                mov     si, xm$heapTail         ;; ds:si-> last allocated heap
                jmp     short @@heap_test

@@heap_loop:    cmp     xm$heapTB[si]._size, eax
                jl      @@heap_next             ;; heapTB[si].size < bytes?

                les     bx, xm$heapTB[si].blkTB ;; es:bx-> blockTB

                mov     di, xm$heapTB[si].fblkHead;; di= 1st free block
...
@@heap_next:    mov     si, xm$heapTB[si].prev  ;; heap= heapTB[heap].prev
@@heap_test:    cmp     si, -1
...
xm$BlockSplit   proc    near public uses ecx bp es
                ...
                cmp     di, xm$heapTB[bp].fblkHead
                jne     @@prev_free
                mov     xm$heapTB[bp].fblkHead, si
                jmp     short @@next_free
...
@@exit:         mov     es:[bx+di].BLK.free,FALSE;; set b as allocated
                ...
                mov     al, B xm$heapTB[si].hnd ;; al= handle
