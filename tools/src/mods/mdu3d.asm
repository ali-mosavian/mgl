...
;;;;;;;;
u3dMtrxCopy     proc    public uses bx di si,\
                        pOut: far ptr real4,\
                        pIna: far ptr real4
...
;;;;;;;;
u3dMtrxByVec3   proc    public uses bx di si,\
                        vOut: far ptr real4,\
                        vOutSize: word, \
                        mIn : far ptr real4,\
                        vIn : far ptr real4,\
                ...
                ;;            v[2]*m[i][2] + m[i][3]
                ;;                
@@loop:         _mtrx_by_vec3   vTmp, gs:[si], fs:[bx]
...
;; ::::::::::::::
;; name: u3dVec3Norm
;; desc: Normalises vectors
;;
;; ::::::::::::::
u3dVec3Norm     proc    public uses bx di si,\
                        vOut: far ptr real4,\
                        vOutSize: word, \
                        vIn : far ptr real4,\
                        vInSize: word, \
                ...
                ;;     {
                ;;         mag = 1.0 / sqrt( mag )
                ;;         vout[i].x = vin[i].x * mag
                ;;         vout[i].y = vin[i].y * mag
                ;;         vout[i].z = vin[i].z * mag
                ;;     }
                ;;     else
                ;;     {
                ;;         vout[i].x = vin[i].x
...
@@loop:         ;;
                ...
                ;;
                fld     D fs:[si+0*4]                   ;; x
                fmul    D fs:[si+0*4]                    ;; x*x
                
                fld     D fs:[si+1*4]                   ;; y x*x
                fmul    D fs:[si+1*4]                   ;; y*y x*x
                
                fld     D fs:[si+2*4]                   ;; z y*y x*x
                fmul    D fs:[si+2*4]                   ;; z*z y*y x*x
                
                fxch    st(2)                           ;; x*x y*y z*z
                faddp   st(1), st(0)                    ;; x*x+y*y z*z
                faddp   st(1), st(0)                    ;; mag
                
                fld     D fs:[si+0*4]                   ;; x mag
                fld     D fs:[si+1*4]                   ;; y x mag
                fld     D fs:[si+2*4]                   ;; z y x mag
                fxch    st(3)                           ;; mag y x z
                ...
                ;; z = z * mag
                ;;
                fmul    st(3), st(0)                    ;; imag z y x
                fmul    st(2), st(0)                    ;; imag z y x
                fmul    st(1), st(0)                    ;; imag z y x
