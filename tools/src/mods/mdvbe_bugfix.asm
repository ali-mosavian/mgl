...
;;::::::::::::::
;; out: ax= value read
vgaRegRead      macro   port:req, index:req
                mov     al, index
                mov     dx, port
                out     dx, al
...
vgaRegWrite     macro   port:req, index:req
                push    ax
                mov     al, index
                mov     dx, port
                out     dx, al
...
;;::::::::::::::
vgaSetLogWidth  proc    near uses bx cx dx,\
                        oldwdth:word, newwdth:word

                vgaRegRead 03d4h, 17h   ;; Get current Mode Control Register
                
                and   al, 10011111b     ;; Strip old W/B and AW fields
                or    al, 00100000b     ;; Word mode and bit 15 wrap (bit 15 wrap is meaningless)
                
                vgaRegWrite 3d4h, 17h   ;; Set new Mode Control Register
                
                mov   ax, newwdth
                shr   ax, 4             ;; Width \ 16

                vgaRegWrite 03d4h, 13h  ;; Program new scanline width in words
...
set_bps         proc    near uses bx cx dx,\
                        xRes:word, yRes:word, pages:word, bps:word, _width:word
...
@@try_serv_00:  cmp     al, 4Fh
                ...
                ;;
                ;; Last resort, try to change scanline width through
                ;; VGA services
                ;;
@@set_vga:      LOGMSG  VGA_set
                cmp     _width, 4096
                jl      @F
...
@@:             invoke  vgaSetLogWidth, xRes, _width
                jmp     @@done
